...
open(my $seq, '<:encoding(UTF-8)', $seqfile) or die "Could not open '$seqfile' $!\n";
...
my $output = $ARGV[1];
open(my $out, '>:encoding(UTF-8)', $output) or die "Could not open file '$output' $!";

print scalar(keys %gnd) . "\n";
